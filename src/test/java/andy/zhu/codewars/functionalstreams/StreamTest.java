...
public class StreamTest {
    ...
    @Test
    public void testFib() {
        Integer[] result = Stream.fibS().takeS(8).toArray(new Integer[8]);
        Integer[] fib = new Integer[] {0, 1, 1, 2, 3, 5, 8, 13};
        for (int i = 0; i < 8; i++) {
            assertEqInt("fib:" + i, result[i], fib[i]);
        }
    }

    @Test
    public void testPrime() {
        Integer[] result = Stream.primeS().takeS(5).toArray(new Integer[5]);
        Integer[] primes = new Integer[] {2, 3, 5, 7, 11};
        for (int i = 0; i < 5; i++) {
            assertEqInt("prime:" + i, result[i], primes[i]);
        }
    }
}
